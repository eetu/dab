...
// it would save. The format own rules are tested in the format package — what is
// only checkable here is that a pointer landing on a canvas cell ends up as the
// right character in the right row.
import { cloneSprite, toJson } from "dab-core";
...
async function clickCell(host: HTMLElement, x: number, y: number, mods: { altKey?: boolean } = {}) {
...
test("⌥ over a brush picks the colour under the cursor and paints nothing", async () => {
  const { host, stop } = await mountApp();
  loadSprite(
    {
      name: "t",
  ...
  await clickCell(host, 1, 0, { altKey: true });
  expect(editor.ink).toBe("B");
  // The tool is where it was — a pick is a detour, not a mode change — and the
  // document is untouched, which is the half a picker that also painted would miss.
  expect(editor.tool).toBe("pencil");
  expect(editor.sprite.frames[0]).toEqual(["AB..", "...."]);

  // And the pick loaded the brush: the next plain click paints what was taken.
  await clickCell(host, 3, 1);
  expect(editor.sprite.frames[0][1]).toBe("...B");
  stop();
});

test("⌥ stays out of select's way, where it means subtract", async () => {
  const { host, stop } = await mountApp();
  loadSprite(
    { name: "t", w: 4, h: 2, palette: { A: "#00ff00" }, frames: [["AA..", "...."]] },
    "t.json",
  );
  editor.ink = "A";
  editor.tool = "select";
  await clickCell(host, 0, 0, { altKey: true });
  // A pick would have left ink alone (it is already A) but changed nothing else;
  // what must not happen is the select tool losing its click to the picker.
