...
// themselves (lift, put down) are tested in the format package, where they live.
...
function centre(host: HTMLElement, x: number, y: number) {
  const canvas = host.querySelector("[data-testid=canvas]");
...
test("a pasted block floats: dragging it off puts back what it covered", async () => {
  await drag(app.host, [[1, 1]]);
  copySelection();
  // Land it squarely on the lone blue pixel at (6,4).
  pasteClipboard({ x: 5, y: 4 });
  expect(rows()[4]).toBe(".....RR.");
  // Shove it up two. The pixel it was sitting on is not a casualty of having
  // been passed over — a paste is not yours until you let go of it.
  await drag(app.host, [
    [5, 4],
    [5, 3],
    [5, 2],
  ]);
  expect(rows()[2]).toBe(".RR..RR.");
  expect(rows()[4]).toBe("......B.");
  // And the whole thing — landing and shoving — is one undo.
  undoEdit();
  expect(rows()[2]).toBe(".RR.....");
  expect(rows()[4]).toBe("......B.");
});

test("a pasted box does not punch its empty corners into what it lands on", async () => {
  // A box round the 2×2 block, one cell of margin: it carries transparency.
  await drag(app.host, [
    [0, 0],
    [3, 3],
  ]);
  copySelection();
  clearSelection();
  // Drop it so the blue pixel at (6,4) falls under one of the empty cells.
  pasteClipboard({ x: 3, y: 2 });
  expect(rows()[3]).toBe("....RR..");
  expect(rows()[4]).toBe("....RRB."); // the B is under a gap, and shows through
});

test("a floating paste says so, in the status bar and on the marquee", async () => {
  expect(floating.on).toBe(false);
  await drag(app.host, [[1, 1]]);
  copySelection();
  expect(app.host.querySelector("[data-testid=afloat]")).toBeNull();

  pasteClipboard({ x: 5, y: 4 });
  await sleep(40);
  expect(floating.on).toBe(true);
  const chip = app.host.querySelector("[data-testid=afloat]");
  expect(chip?.textContent).toContain("Floating");
  // The ants agree with the chip: one statement, said in two places.
  expect(app.host.querySelector(".ants")?.classList.contains("floating")).toBe(true);

  // Drop puts it down where it sits and stops saying anything. The marquee
  // stays: letting go of the float is not letting go of the selection.
  (chip?.querySelector("button") as HTMLButtonElement).click();
  await sleep(40);
  expect(floating.on).toBe(false);
  expect(hasSelection()).toBe(true);
  expect(app.host.querySelector("[data-testid=afloat]")).toBeNull();
  expect(app.host.querySelector(".ants")?.classList.contains("floating")).toBe(false);
  // And it really is baked: moving it now leaves the hole a move leaves.
  await drag(app.host, [
    [5, 4],
    [5, 2],
  ]);
...
test("a move does not claim to be floating — nothing is under it to lose", async () => {
  await drag(app.host, [[1, 1]]);
  await drag(app.host, [
    [1, 1],
    [3, 1],
...
test("drawing lets go of a floating paste, and the stroke survives a nudge", async () => {
  await drag(app.host, [[1, 1]]);
  copySelection();
  pasteClipboard({ x: 5, y: 4 });
  expect(floating.on).toBe(true);
  // Draw over the pasted block. Keeping the float here would leave the next
  // nudge stamping onto a base taken BEFORE the stroke, rubbing it out.
  editor.tool = "pencil";
  editor.ink = "B";
  paint([[5, 4]], true);
  expect(floating.on).toBe(false);
  expect(rows()[4]).toBe(".....BR.");
  editor.tool = "select";
  nudgeSelection(0, -2);
